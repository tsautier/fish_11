...
impl ConfigKey {
    ...
    pub fn encrypt_config(&self, config_data: &str) -> Result<String, String> {
        ...
        cipher
            .encrypt(nonce, config_data.as_bytes())
    ...
    pub fn rotate(&mut self, master_key: &MasterKey) {
        self.key_material =
            master_key.derive_subkey(&format!("config_encryption_v{}", self.version + 1));
        self.version += 1;
    }
}
...
impl ConfigKeyGuard {
    pub fn new_from_master(master_key: &MasterKey) -> Self {
        Self { inner: Arc::new(Mutex::new(ConfigKey::new_from_master(master_key))) }
    }
