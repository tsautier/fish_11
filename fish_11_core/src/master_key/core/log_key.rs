...
impl LogKey {
    ...
    pub fn encrypt_log_entry(&self, log_entry: &str) -> Result<String, String> {
        ...
        cipher.encrypt(nonce, log_entry.as_bytes())
    ...
    pub fn decrypt_log_entry(&self, encrypted_log: &str) -> Result<String, String> {
        use chacha20poly1305::Nonce;
        let encrypted_bytes = base64::decode(encrypted_log)
    ...
    pub fn needs_rotation(&self) -> bool {
        ...
        match self.rotation_policy {
            ...
            LogRotationPolicy::AfterSize(size_limit) => {
    ...
    pub fn rotate(&mut self, master_key: &MasterKey) {
        self.key_material = master_key.derive_subkey(&format!("log_encryption_v{}", self.version + 1));
        self.version += 1;
        self.current_size = 0;
...
impl LogKeyGuard {
    pub fn new_from_master(master_key: &MasterKey) -> Self {
        Self {
            inner: Arc::new(Mutex::new(LogKey::new_from_master(master_key))),
        }
    }
