...
pub fn encrypt_config_data(data: &str, master_key: &[u8; 32]) -> Result<Vec<u8>, String> {
    ...
    let encrypted_blob = encrypt_data(data.as_bytes(), &config_key, "config", 0)
    ...
    let base64_data = general_purpose::STANDARD.encode(&encrypted_bytes);
...
pub fn decrypt_config_data(encrypted_data: &[u8], master_key: &[u8; 32]) -> Result<String, String> {
...
pub fn is_encrypted_config(data: &[u8]) -> bool {
    ...
    if let Ok(content) = String::from_utf8(data.to_vec()) {
        content.lines().next().map_or(false, |line| line.starts_with(ENCRYPTED_CONFIG_HEADER))
    } else {
        false
    }
}
