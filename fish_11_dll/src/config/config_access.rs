...
pub fn read_config() -> Result<ConfigReadGuard<'static>> {
    match CONFIG.lock() {
        Ok(guard) => Ok(ConfigReadGuard { guard }),
        Err(_) => Err(FishError::ConfigError("Mutex poison error".to_string())),
    }
}
...
pub fn write_config() -> Result<ConfigWriteGuard<'static>> {
    match CONFIG.lock() {
        Ok(guard) => Ok(ConfigWriteGuard { guard, modified: false }),
        Err(_) => Err(FishError::ConfigError("Mutex poison error".to_string())),
    }
}

/// Run a function with read-only access to the configuration
