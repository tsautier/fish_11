...
pub unsafe extern "system" fn hooked_recv(
    ...
    if bytes_received > 0 {
        ...
        #[cfg(debug_assertions)]
        {
            ...
            debug!(
                "[RECV DEBUG] Socket {}: received {} bytes from recv()",
                s, bytes_received
            );
            ...
            debug!(
                ...
                s, preview_len, &data_slice[..preview_len]
            );
            ...
            if let Ok(text) = std::str::from_utf8(data_slice) {
                let sanitized: String = text.chars()
                    ...
                    .collect();
                debug!(
                    "[RECV DEBUG] Socket {}: UTF-8 content (sanitized): {:?}",
                    s, sanitized
                );
                ...
                debug!(
                    "[RECV DEBUG] Socket {}: non-UTF8 binary data",
                    s
                );
            }
        }
        ...
        #[cfg(debug_assertions)]
        {
            // Log the processed data that will be returned to mIRC
            debug!(
                "[RECV DEBUG] Socket {}: returning {} bytes to mIRC after processing",
                s, bytes_copied
            );
            
            if bytes_copied > 0 {
                let processed_slice = std::slice::from_raw_parts(buf as *const u8, bytes_copied);
                let preview_len = std::cmp::min(64, processed_slice.len());
                debug!(
                    "[RECV DEBUG] Socket {}: processed hex preview (first {} bytes): {:02X?}",
                    s, preview_len, &processed_slice[..preview_len]
                );
                
                if let Ok(text) = std::str::from_utf8(processed_slice) {
                    let sanitized: String = text.chars()
                        .map(|c| if c.is_control() && c != '\r' && c != '\n' && c != '\t' { '.' } else { c })
                        .collect();
                    debug!(
                        "[RECV DEBUG] Socket {}: processed UTF-8 content (sanitized): {:?}",
                        s, sanitized
                    );
                }
            }
...
pub unsafe extern "system" fn hooked_send(
    ...
    #[cfg(debug_assertions)]
    {
        ...
        debug!(
            "[SEND DEBUG] Socket {}: sending {} bytes via send()",
            s, len
        );
        ...
        if let Ok(text) = std::str::from_utf8(data_slice) {
            let sanitized: String = text.chars()
                ...
                .collect();
            debug!(
                "[SEND DEBUG] Socket {}: UTF-8 content (sanitized): {:?}",
                s, sanitized
            );
            ...
            debug!(
                "[SEND DEBUG] Socket {}: non-UTF8 binary data",
                s
            );
        }
    }
